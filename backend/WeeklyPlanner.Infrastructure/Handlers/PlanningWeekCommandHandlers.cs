...
public class FreezePlanningWeekHandler : IRequestHandler<FreezePlanningWeekCommand, Result<PlanningWeekDto>>
{
    ...
    public async Task<Result<PlanningWeekDto>> Handle(FreezePlanningWeekCommand request, CancellationToken cancellationToken)
    {
        try
        {
            ...
            if (planningWeek.IsFrozen)
            {
                return Result<PlanningWeekDto>.Fail("Planning week is already frozen");
            }

            // Load all week members with their tasks for validation
            var weekMembers = await _dbContext.WeekMembers
                .Include(wm => wm.Tasks)
                .Where(wm => wm.WeekId == request.Id)
                .ToListAsync(cancellationToken);

            if (weekMembers.Count == 0)
            {
                return Result<PlanningWeekDto>.Fail("Cannot freeze: no members assigned to this week");
            }

            // Validate every member has exactly 30h planned
            var membersNot30h = weekMembers.Where(wm => wm.TotalPlannedHours != 30).ToList();
            if (membersNot30h.Count > 0)
            {
                return Result<PlanningWeekDto>.Fail($"Cannot freeze: {membersNot30h.Count} member(s) do not have exactly 30h planned");
            }

            // Validate every member has submitted ("I'm Done Planning")
            var membersNotSubmitted = weekMembers.Where(wm => !wm.HasSubmitted).ToList();
            if (membersNotSubmitted.Count > 0)
            {
                return Result<PlanningWeekDto>.Fail($"Cannot freeze: {membersNotSubmitted.Count} member(s) have not marked as done planning");
            }

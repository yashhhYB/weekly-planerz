...
public class DeleteTeamMemberHandler : IRequestHandler<DeleteTeamMemberCommand, Result<bool>>
{
    ...
    public async Task<Result<bool>> Handle(DeleteTeamMemberCommand request, CancellationToken ct)
    {
        try
        {
            ...
            // Check if member is assigned to an active week (Setup or InProgress)
            var isInActiveWeek = await _db.WeekMembers
                .Include(wm => wm.Week)
                .AnyAsync(wm => wm.MemberId == request.Id && (wm.Week.Status == 1 || wm.Week.Status == 2), ct);

            if (isInActiveWeek)
                return Result<bool>.Fail("Cannot remove member while weekly planning is active. Complete or cancel the current week first.");

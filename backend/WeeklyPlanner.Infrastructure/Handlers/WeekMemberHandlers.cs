...
public class UnsubmitMemberPlanHandler : IRequestHandler<UnsubmitMemberPlanCommand, Result<WeekMemberDto>>
{
    private readonly ApplicationDbContext _db;

    public UnsubmitMemberPlanHandler(ApplicationDbContext db) { _db = db; }

    public async Task<Result<WeekMemberDto>> Handle(UnsubmitMemberPlanCommand request, CancellationToken ct)
    {
        var weekMember = await _db.WeekMembers
            .Include(wm => wm.Member)
            .Include(wm => wm.Week)
        ...
        if (weekMember == null) return Result<WeekMemberDto>.Fail("Week member not found");
        if (weekMember.Week.IsFrozen) return Result<WeekMemberDto>.Fail("Cannot unsubmit — planning week is frozen");
        ...
        }, "Plan unsubmitted — you can edit again");
    }
}
